...
function needsReprocess(video: VideoData): boolean {
  return !video.video_url ||
    ...
    (!video.video_url.includes("supabase.co") &&
...
function CompletedCard({
  ...
  const handleReprocess = useCallback(
    async (e: React.MouseEvent) => {
      e.stopPropagation();
      if (reprocessing || !video.job_id) return;
      setReprocessing(true);
      try {
        await onReprocess(video.job_id);
      } finally {
...
export default function GalleryPage() {
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      ...
      if (brokenCompleted.length > 0) {
        const target = brokenCompleted[0];
        autoReprocessing.add(target.job_id);
        fetch(`/api/videos/${target.job_id}/reprocess`, {
          method: "POST",
          headers: { Authorization: `Bearer ${session.access_token}` },
        }).catch(() => {
        });
  ...
  const handleReprocess = useCallback(
    async (jobId: string) => {
      if (!session?.access_token) return;
      try {
        const res = await fetch(`/api/videos/${jobId}/reprocess`, {
          method: "POST",
          headers: { Authorization: `Bearer ${session.access_token}` },
        });
        if (res.ok) {
          ...
          console.error("Reprocess failed:", data.error || res.statusText);
        }
      } catch (err) {
