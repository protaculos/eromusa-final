            ...
            <div key={template.id} className="shrink-0 w-[130px] sm:w-[200px] relative group/card">
              ...
              {isAdmin && (
                <div className="absolute inset-0 z-30 pointer-events-none">
                  {/* 🗑️ Delete scene — top left */}
                  {onDeleteScene && (
                    <button
                      onClick={(e) => { e.preventDefault(); e.stopPropagation(); onDeleteScene(template.id); }}
                      className="pointer-events-auto absolute top-0 left-0 -translate-x-1/2 -translate-y-1/2 w-7 h-7 rounded-full bg-black/90 flex items-center justify-center hover:bg-red-500 transition-colors shadow-lg border border-white/20"
