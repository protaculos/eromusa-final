/**
 * Script de Migração: Pastas public/videos/ + styles.json → Supabase
 *
 * Uso:
 *   1. Execute o SQL em scripts/001-schema.sql no Supabase Dashboard
 *   2. Crie o bucket "scene-videos" no Supabase Storage (público)
 *   3. Configure SUPABASE_URL e SUPABASE_SERVICE_ROLE_KEY no .env.local
 *   4. npx tsx scripts/migrate-scenes.ts
 *
 * O que faz:
 *   - Lê todas as pastas em public/videos/ que contêm video.webm
 *   - Mapeia cada pasta ao style correspondente em src/data/styles.json
 *   - Faz upload do video.webm para Supabase Storage (bucket: scene-videos)
 *   - Cria registro na tabela scenes
 *   - Cria categorias iniciais e vincula as cenas
 */
...
async function main() {
  ...
  const allStyles: StyleRaw[] = stylesRaw.video_styles || stylesRaw;
  ...
  const styleBySlug: Map<string, StyleRaw> = new Map();
  ...
  for (const folder of folders) {
    ...
    try {
      ...
      if (uploadError) {
        console.error(`  ❌ ${folder}: upload failed - ${uploadError.message}`);
        errors++;
        continue;
      }
      ...
      const { error: dbError } = await supabase
        ...
        .insert({
          ...
          style_id: style.id,
          loop_video_url: urlData.publicUrl,
          gradient,
        });

      if (dbError) {
        console.error(`  ❌ ${folder}: DB insert failed - ${dbError.message}`);
        errors++;
        continue;
      }

      console.log(`  ✅ ${style.name} (${folder}) → ${urlData.publicUrl}`);
      uploaded++;
    } catch (err) {
  ...
  console.log(`  2. Add public/videos/ to .gitignore`);
